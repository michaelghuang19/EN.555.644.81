...
\begin{itemize}
  ...
  \item A \textbf{put} gives its long the \emph{right to sell} the underlying at a fixed strike.

  \item A \textbf{call} gives its long the \emph{right to buy} at a fixed strike.

\end{itemize}

The company will be \emph{selling} the foreign currency for home currency when it arrives. To protect against the FC weakening while keeping the upside if the FC strengthens, the company wants the \emph{right} (not the obligation) to sell FC at a guaranteed minimum exchange rate. \\ \\ \\
The put option contract is most appropriate for hedging (strike expressed as units of home currency per unit of FC), expiring around the expected receipt date. The put establishes a floor on the realized exchange rate; if FC rallies, let the option expire and sell at the (better) spot rate. This trades a premium today for asymmetric upside, exactly the trade-off highlighted in M1A slide 1.12.
